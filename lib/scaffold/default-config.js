...
/**
 * Will return the path and default trivechaincore-node configuration. It will search for the
 * configuration file in the "~/.trivechaincore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.trivechaincore');
  var defaultConfigFile = path.resolve(defaultPath, './trivechaincore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['trivechaind', 'web'];
