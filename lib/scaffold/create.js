...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'bitcore-lib-dash': 'git://git@gitlab.com:Thunderlabtech/bitcore-lib-dash.git',
    'bitcore-node-dash': "git://git@gitlab.com:Thunderlabtech/bitcore-node-dash.git"
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-dash.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitcore-node-dash directory and configuration
