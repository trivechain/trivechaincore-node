...
var BASE_PACKAGE = {
  description: 'A full Trivechain node build with Trivechaincore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'trivechaincore-lib': 'git://git@github.com:trivechain/trivechaincore-lib.git',
    'trivechaincore-node': 'git://git@github.com:trivechain/trivechaincore-node.git'
  }
};

/**
 * Will create a directory and trivechain.conf file for Trivechain.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createTrivechainDirectory(datadir, done) {
...
/**
 * Will create a base Trivechaincore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The trivechain database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/trivechaincore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Trivechaincore Node directory, configuration file,
 * trivechain configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the trivechaincore node configuration directory
 * @param {String} options.datadir - The path to the trivechain datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the trivechaincore-node directory and configuration
    ...
    function(next) {
      // Setup the Trivechain directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createTrivechainDirectory(absDataDir, next);
      } else {
        next();
      }
    },
