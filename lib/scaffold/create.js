...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'bitcore-lib-trvc': 'git://git@gitlab.com:Thunderlabtech/bitcore-lib-trvc.git',
    'bitcore-node-trvc': "git://git@gitlab.com:Thunderlabtech/bitcore-node-trvc.git"
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-trvc.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitcore-node-trvc directory and configuration
