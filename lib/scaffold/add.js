...
function addService(configDir, service, done) {
  ...
  var npm = spawn('npm', ['install', "git+ssh://git@github.com:trivechain/"+service+".git", '--save'], {cwd: configDir});
 ...
 * @param {String} options.dirname - The trivechaincore-node configuration directory
...
function add(options, done) {
  ...
  var trivechaincoreConfigPath = path.resolve(configPath, 'trivechaincore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(trivechaincoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a trivechaincore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, "git+ssh://git@github.com:trivechain/"+service+".git", function(err) {
        if (err) {
          return next(err);
        }
        ...
        // add service to trivechaincore-node.json
        addConfig(trivechaincoreConfigPath, serviceName, next);
      });
    }, done
  );
}
