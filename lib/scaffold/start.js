...
var TrivechaincoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for trivechaind is necessary for this upgrade with the "reindex=1" trivechain.conf option. \n' +
                 'There are changes necessary in both trivechain.conf and trivechaincore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/trivechain/trivechaincore-node/blob/trivechaind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          trivechaind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/trivechaind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed trivechaincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './trivechaincore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in trivechaincore-node services directory
    ...
    if (servicePackage.trivechaincoreNode) {
      serviceModule = service.name + '/' + servicePackage.trivechaincoreNode;
    }
 ...
 * @param {Node} node - The Trivechaincore Node instance
 */
