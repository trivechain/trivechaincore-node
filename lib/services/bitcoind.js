...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './trivecoin.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-node-dash. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-node-dash. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-node-dash. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that dashd is undergoing a reindex. ' +
             ...
             'of bitcore-node-dash services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  ...
  if(!blockArg)
  {
  	return callback(self._wrapRPCError(new Error("Block unavailable")));
  }
