...
/**
 * Provides a friendly event driven API to trivechaind in Node.js. Manages starting and
 * stopping trivechaind as a child process for application support, as well as connecting
 * to multiple trivechaind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Trivechain(options) {
  if (!(this instanceof Trivechain)) {
    return new Trivechain(options);
  }
...
util.inherits(Trivechain, Service);

Trivechain.dependencies = [];

Trivechain.DEFAULT_MAX_TXIDS = 1000;
Trivechain.DEFAULT_MAX_HISTORY = 50;
Trivechain.DEFAULT_MAX_UTXO = 1000;
Trivechain.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Trivechain.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Trivechain.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Trivechain.DEFAULT_SPAWN_RESTART_TIME = 5000;
Trivechain.DEFAULT_SPAWN_STOP_TIME = 10000;
Trivechain.DEFAULT_TRY_ALL_INTERVAL = 1000;
Trivechain.DEFAULT_REINDEX_INTERVAL = 10000;
Trivechain.DEFAULT_START_RETRY_INTERVAL = 5000;
Trivechain.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Trivechain.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Trivechain.DEFAULT_DIRECTSEND_FEE = 10000;
Trivechain.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'trivechaincore'
};

Trivechain.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Trivechain.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Trivechain.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Trivechain.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Trivechain.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Trivechain.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Trivechain.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Trivechain.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Trivechain.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Trivechain.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Trivechain.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Trivechain.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Trivechain.prototype._initCaches = function() {
...
Trivechain.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Trivechain.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Trivechain.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'trivechaind/transactionlock',
      scope: this,
...
Trivechain.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'trivechaind/' + name, 'total:', this.subscriptions[name].length);
};

Trivechain.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'trivechaind/' + name, 'total:', this.subscriptions[name].length);
};

Trivechain.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (trivechaincore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'trivechaind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Trivechain.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'trivechaind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Trivechain.prototype.unsubscribeAddressAll = function(emitter) {
...
Trivechain.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Trivechain.DEFAULT_CONFIG_SETTINGS;
...
Trivechain.prototype._parseTrivechainConf = function(configPath) {
  var options = {};
...
Trivechain.prototype._expandRelativeDatadir = function() {
...
Trivechain.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in trivechaind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in trivechaind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in trivechaind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './trivechain.conf');

  log.info('Using Trivechain config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseTrivechainConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseTrivechainConf(networkConfigPath));
  }
...
Trivechain.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of trivechaincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of trivechaincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of trivechaincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to trivechaind from trivechaincore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from trivechaind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
      spawnConfig.zmqpubrawtxlock,
      '"zmqpubrawtxlock" option is required to get transaction locks from trivechaind. ' +
      'Please add "zmqpubrawtxlock=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from trivechaind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in trivechain.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that trivechaind is undergoing a reindex. ' +
             ...
             'of trivechaincore-node services will start.');
    node._reindex = true;
  }
};

Trivechain.prototype._resetCaches = function() {
 ...
 * @private
 */
Trivechain.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Trivechain.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Trivechain.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Trivechain Daemon Ready');
          callback();
        });
      });
...
Trivechain.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === trivechaincore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Trivechain.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === trivechaincore.Networks.testnet) {
    networkPath = 'testnet3/trivechain.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/trivechain.conf';
    }
  }
  return networkPath;
};

Trivechain.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === trivechaincore.Networks.testnet) {
...
Trivechain.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('trivechaind/hashblock', message.toString('hex'));
    }
  }

};

Trivechain.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Trivechain.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Trivechain Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Trivechain.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Trivechain.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('trivechaind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Trivechain.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = trivechaincore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('trivechaind/rawtransaction', message.toString('hex'));
    }

    var tx = trivechaincore.Transaction();
    tx.fromString(message);
    var txid = trivechaincore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Trivechain.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = trivechaincore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('trivechaind/transactionlock', message.toString('hex'));
    }

  }
};

Trivechain.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Trivechain.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Trivechain.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Trivechain.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Trivechain.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Trivechain Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Trivechain.DEFAULT_REINDEX_INTERVAL);

};

Trivechain.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Trivechain.prototype._stopSpawnedTrivechain = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/trivechaind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned trivechain process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean trivechain process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Trivechain.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedTrivechain(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting trivechain process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Trivechain process unexpectedly exited with code:', code);
        log.warn('Restarting trivechain child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Trivechain process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new TrivechaindRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn trivechaind.'));
      }
...
Trivechain.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to trivechaind.'));
    }
 ...
 */
Trivechain.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Trivechain configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Trivechain.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Trivechain.prototype.syncPercentage = function(callback) {
  var self = this;
...
Trivechain.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, trivechaincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Trivechain.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = trivechaincore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Trivechain.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Trivechain.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Trivechain.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in trivechaincore is most recent to less recent
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Trivechain.prototype._getConfirmationsDetail = function(transaction) {
...
Trivechain.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Trivechain.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Trivechain.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A trivechain transaction id
 * @param {Function} callback
 */
Trivechain.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Trivechain.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof trivechaincore.Address) {
...
Trivechain.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Trivechain.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Trivechaincore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Trivechain.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = trivechaincore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Trivechain.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
  ...
  if (_.isFunction(options) && _.isUndefined(callback)) {
    ...
    if(options.hasOwnProperty('isDirectSend')){
	    isDirectSend = options.isDirectSend;
    }
  }

  this.client.sendRawTransaction(tx, allowAbsurdFees, isDirectSend, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
Trivechain.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Trivechaincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Trivechain.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Trivechain.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Trivechain.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Trivechain.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Trivechain.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Trivechain.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Trivechain.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Trivechain.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Trivechain.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Trivechain.prototype.govCount = function (callback) {
  var self = this;
...
Trivechain.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Trivechain.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Trivechain.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Trivechain.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.getInfo = function(callback) {
  var self = this;
...
Trivechain.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Trivechain.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('trivechaind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('trivechaind process did not exit'));
      }
