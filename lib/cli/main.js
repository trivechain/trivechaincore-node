...
var trivechaincorenode = require('..');
var utils = require('../utils');

function main(servicesPath, additionalServices) {
  /* jshint maxstatements: 100 */

  var version = trivechaincorenode.version;
  var create = trivechaincorenode.scaffold.create;
  var add = trivechaincorenode.scaffold.add;
  var start = trivechaincorenode.scaffold.start;
  var remove = trivechaincorenode.scaffold.remove;
  var callMethod = trivechaincorenode.scaffold.callMethod;
  var findConfig = trivechaincorenode.scaffold.findConfig;
  var defaultConfig = trivechaincorenode.scaffold.defaultConfig;
  ...
  program
    ...
    .option('-d, --datadir <dir>', 'Specify the Trivechain database directory')
  ...
  program
    ...
    .option('-d, --daemon', 'Make trivechaincore-node a daemon (running in the background)')
    .action(function(cmd){
      if (cmd.config) {
  ...
  program
    ...
    .action(function(services){
      ...
      if (!configInfo) {
        throw new Error('Could not find configuration, see `trivechaincore-node create --help`');
      }
      var opts = {
      ...
      console.log();
      console.log('    $ trivechaincore-node add wallet-service');
      console.log('    $ trivechaincore-node add insight-api');
      console.log();
    });
  ...
  program
    ...
    .action(function(services){
      ...
      console.log('  Examples:');
      console.log();
      console.log('    $ trivechaincore-node remove wallet-service');
      console.log('    $ trivechaincore-node remove insight-api');
      console.log();
    });
